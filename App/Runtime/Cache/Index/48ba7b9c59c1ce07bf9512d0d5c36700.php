...
<!--		<div class="header">
			<div class="inner">
				...
				<ul class="nav">
					...
					<li class="on"><a href="<?php echo U('immigrant/immigrant');?>">移民</a></li>
		...
		<!--签证开始-->
		<div class="visa yimin">
			<div class="inner">
				<div class="tit"><span class="tit-cnt">热门项目</span></div>
				<div class="tip">通过风险评估体系，延续成功经验，同业领先</div>
                                <ul id="list" class="list">
                                    <?php if(is_array($list)): foreach($list as $key=>$v): ?><li><a href="<?php echo U('visa/visadetail?nav=3&proid='.$v['id']);?>">
...
//                        //发送ajax请求将分类id和分页参数传递到后台
                         var url = "<?php echo U('Immigrant/ajaxImm');?>";
                        $('.more').click(function(){
                            ofset = ofset+3;
                            $.post(url,
                            {
...
//                                alert(data);
                                if(data){
                                    $.each(data,function(key,v){
                                        $('#list').append('<li><a href="<?php echo U('Visa/visadetail?nav=3');?>/proid/'+v.id+'"><img src="/jdzx/Public/Uploads/'+v.logo+'"/><div class="mask"><p class="cn">'+v.title+'</p><p class="en">'+v.entitle+'</p></div></a></li>');
                                    });
                                }
                            });
                        });
