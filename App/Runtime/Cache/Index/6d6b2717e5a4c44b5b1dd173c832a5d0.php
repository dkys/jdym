...
<!--		<div class="header">
			<div class="inner">
				...
				<ul class="nav">
					...
					<li class="on"><a href="<?php echo U('case/cases');?>">案例</a></li>
		...
		<div class="case">
			<div class="inner">
				<div class="title">
					...
					<ul class="nav">
						<?php if(is_array($cateinfo)): foreach($cateinfo as $key=>$vo): ?><li class="list" id="<?php echo ($vo["id"]); ?>"><a href="<?php echo U('case/cases?cateid='.$vo['id']);?>"><?php echo ($vo["cate_name"]); ?></a></li><?php endforeach; endif; ?>
					</ul>
				</div>	
				<ul id='list' class="list clear">
					<?php if(is_array($info)): $i = 0; $__LIST__ = array_slice($info,0,8,true);if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><li><a href="<?php echo U('case/casedetail?newsid='.$v['id']);?>">
						<div class="pic">
						...
						<div class="infor">
							<p class="line"></p>
							<p class="cn"><?php echo ((isset($v["title"]) && ($v["title"] !== ""))?($v["title"]):"暂无内容"); ?></p>
...
//                        alert(cateid);
                        ...
                        var url = "<?php echo U('case/ajaxNewsInfo');?>";
                        $('.more').click(function(){
                            ofset = ofset+8;
                            $.post(url,
                            {
...
//                                alert(data.error);
                                if(!data.error){
                                    $.each(data,function(key,v){
                                        $('#list').append('<li><a href="<?php echo U('case/casedetail');?>'+'?newsid='+v.id+'"><div class="pic"><img src="/jdzx/Public/Uploads/'+v.sm_logo+'"/><div class="mask"></div></div><div class="infor"><p class="line"></p><p class="cn">'+v.title+'</p><p class="en">'+v.entitle+'</p></div></a></li>');
                                    });
                                }
//                                else{
